...
int randomizePartion(int *A, int l, int r){
	int random_number = l+(rand()%(l-r));
	int temp = A[random_number];
	A[random_number] = A[r];
	A[r] = temp;

	return partitionAboutPivot(A,l,r);
}

int partitionAboutPivot(int *A, int l, int r){
...
void quickSort(int *A, int l, int r){
	if (l<r)
	{
		int pivot = randomizePartion(A,l,r);
